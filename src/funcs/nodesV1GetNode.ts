 ...
 */
export function nodesV1GetNode(
...
> {
  const input: operations.GetNodeRequest = {
    nodeId: nodeId,
  };

  const parsed = safeParse(
    input,
    (value) => operations.GetNodeRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/nodes/v1/{nodeId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.NodeV1$inboundSchema),
