 ...
 */
export function nodesV1ListProvisionedNodes(
...
> {
  const input: operations.ListProvisionedNodesRequest = {
    fleetId: fleetId,
    orgId: orgId,
    region: region,
  };
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.ListProvisionedNodesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/nodes/v1/fleet/{fleetId}/listProvisioned")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, z.array(components.NodeV1$inboundSchema)),
