 ...
 * GetUsageLimits
 */
export async function organizationsV1GetUsageLimits(
  client: HathoraCloudCore,
  orgId?: string | undefined,
  options?: RequestOptions,
): Promise<
  Result<
    components.UsageLimits,
    | errors.ApiError
    | errors.ApiError
    | SDKError
    | SDKValidationError
  ...
  >
> {
  const input: operations.GetUsageLimitsRequest = {
    orgId: orgId,
  };

  const parsed = safeParse(
    input,
    (value) => operations.GetUsageLimitsRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.UsageLimits$inboundSchema),
